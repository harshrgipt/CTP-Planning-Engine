...
def main() -> None:
    ...
    def _place(p: str, gt: str, cand: list, grp: list, cap_h: float) -> bool:
            ...
            for mach in cand:
                ...
                en = st + dur
                if st < t0:
                    _k = 'before_t0_IDEAL' if ideal < t0 else 'before_t0_CONTENTION'
                    _WHY[(p,_k)] = _WHY.get((p,_k),0)+1
                    _sh = (t0 - st).total_seconds()/3600.0
                    _WHY[(p,_k+'_h')] = _WHY.get((p,_k+'_h'),0.0)+_sh
                    continue
                ...
                if (ideal - st).total_seconds() / 3600.0 > cap_h:
                    _WHY[(p,'earliness_cap')] = _WHY.get((p,'earliness_cap'),0)+1
                    continue
                ...
                if worst > GT_SHELF_LIFE_H:
                    _WHY[(p,'R5_shelf_life')] = _WHY.get((p,'R5_shelf_life'),0)+1
                    continue
                ...
                if not _cap_ok(p, _adds):
                    _WHY[(p,'wip_rail')] = _WHY.get((p,'wip_rail'),0)+1
                    continue
                if PIN_RUNS:
                ...
                if best is None or key < (-(t_first - best[3]).total_seconds()
                    ...
                    best = (mach, c, st, en)
            if best is None:
                _WHY[(p,'RUN_FAILED')] = _WHY.get((p,'RUN_FAILED'),0)+1
                _WHY[(p,'RUN_FAILED_q')] = _WHY.get((p,'RUN_FAILED_q'),0)+gq
                return False
            mach, c, st, en = best
    ...
    print(f"  [WHY] machine-rejection reasons: {_WHY}")
