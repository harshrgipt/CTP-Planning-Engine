...
# DIAGNOSTIC ONLY -- see `owners()`. off | free | tyres
BALANCE = __import__("os").environ.get("PLANNER_PART_BALANCE", "off")
...
def main() -> None:
    ...
    for P in ("PCR", "TBR"):
            ...
            for m in pool:
                ...
                for i, (rr, sh) in enumerate(sizes[(P, m)]):
                    if rr == g["rim"]:
                        ...
                        # BALANCE MODE (diagnostic). `-free[m]` already sat in
                        # this key but BEHIND `-sh`, and `sh` is a continuous
                        # 8-month share that essentially never ties -- so the
                        # only load-balancing term in the whole seater was dead.
                        # A machine at 100 % historical purity absorbs GTs until
                        # it is full while an 89 %-purity machine on the SAME rim
                        # with the SAME cadence is left half-booked.
                        #   off    (shipped) share first, free hours as tie-break
                        #   free              free HOURS first  -> worst-fit
                        #   tyres             free TYRE capacity -> favours fast
                        if BALANCE in ("free", "bal"):
                            key = (i, diff_min(P, m) if i else 0.0,
                                   -free[m], -sh, m)
                        elif BALANCE == "tyres":
                            key = (i, diff_min(P, m) if i else 0.0,
                                   -free[m] * 3600.0 / cad.get(
                                       m, CAD_DEFAULT[P]), -sh, m)
                        else:
                            key = (i, diff_min(P, m) if i else 0.0,
                                   -sh, -free[m], m)
                        out.append(key)
        ...
        # DIAGNOSTIC: `merge`/`bal` collapse tiers 1 and 2 into ONE pass.
        # The tier loop is EXHAUSTIVE -- every tier-1 machine is filled to
        # PART_UTIL before a tier-2 machine is offered its first GT -- so a
        # tier-2 machine that owns the rim as its PRIMARY size sits half-booked
        # beside a saturated tier-1 machine on the same rim. Measured July PCR:
        # the two lowest-booked machines (TBMPCR5 56 %, TBMPCR8 72 %) are
        # EXACTLY the two tier-2 machines, and TBMPCR5 is a 49 s machine.
        # Purity does not depend on the tier order -- it is held by `locked`
        # and by the `i` (primary-size) term inside `owners()`, both of which
        # still apply -- so merging cannot let a machine take a size it never ran.
        _tiers = ([(t1 + t2, 1), (t3, 3)] if BALANCE in ("merge", "bal")
                  else [(t1, 1), (t2, 2), (t3, 3)])
        for pool, tg in _tiers:
            still = []
            for g in pending:
